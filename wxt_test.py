...
print(WXT520.find_wxt())
time.sleep(1)
with WXT520('COM5', address='0', timeout=10) as w:
    t = time.time()
    while time.time()-t < 600:
        line = w.readline()
        if line:
            d = WXT520.message_to_dict(line)
            if d['Type'] == 'PTU':
                def valueprint(pair):
                    return "{} {}".format(*pair)
                print(valueprint(d['Data']['Temperature']['Ambient']),
                      '--', valueprint(d['Data']['Humidity']))
